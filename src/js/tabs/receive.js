...
ReceiveTab.prototype.angular = function(module)
{
	module.controller('ReceiveCtrl', [
			...
			function($scope, $id, $rpTracker)
			{
				...
				// watch the address function and detect when it changes so we
				// can inject the qr
				$scope.$watch('address', function()
				{
					if ($scope.address !== undefined)
						...
						$('#qr-code').qrcode(
								'https://latcoin.com//contact?to='
										+ $scope.address);
				}, true);

				// XXX: This used to be in onAfterRender. Refactor and re-enable
				/*
				 * this.el.find('.btn').click(function (e) { e.preventDefault();
				 * 
				 ...
				 * 
				 * function highlightAnimation() { // Trigger highlight
				 * animation self.el.find('.address') .stop()
				 * .css('background-color', '#fff') .effect('highlight', {
				 * color: "#8BC56A", easing: jQuery.easing.easeOutSine() },
				 * 800); }
				 */

				$rpTracker.track('Page View',
				{
					'Page Name' : 'Receive'
				});
			} ]);
