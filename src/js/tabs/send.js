...
SendTab.prototype.angular = function (module)
{
                                           ...
                                           $network, $federation, $rpTracker)
  {
    ...
    // LAT currency object.
    // {name: "LAT - Latcoins", order: 146, value: "LAT"}
    $scope.xrp = _.where($scope.currencies_all, {value: "LAT"})[0];
    ...
    $scope.$watch('send.currency', function () {
      $scope.send.currency_code = $scope.send.currency ? $scope.send.currency.slice(0, 3).toUpperCase() : "LAT";
    ...
    $scope.update_destination = function () {
      ...
      // Trying to send LAT to self.
      // This is used to disable 'Send LAT' button
    ...
    // Check destination for LAT sufficiency and flags
    $scope.check_destination = function () {
      ...
      account.entry(function (e, data) {
        $scope.$apply(function () {
          ...
          // If we get this far, we have a Latcoin address resolved
    ...
    $scope.update_currency_constraints = function () {
      ...
      if (send.recipient_info.exists && send.recipient_lines) {
        ...
        // Add LAT if they allow it
        if (!send.recipient_info.disallow_xrp) {
          send.currency_choices.unshift("LAT");
        }
        // ---------------------------------------------------------------------
        ...
        // If the account doesn't exist, we can only send LAT
        send.currency_choices = ["LAT"];
        send.currency_force = "LAT";
        send.currency = "LAT";
      }
    ...
    $scope.update_amount = function () {
      ...
      if (send.quote_url) {
        ...
        // Cannot make LAT payment if the sender does not have enough LAT
            ...
            $scope.send.alternatives = _.map(upd.alternatives, function (raw,key) {
              ...
              if (raw.source_amount.currency) {
                if (raw.source_amount.currency === $scope.send.currency_code)
                  currentKey = key;
              } else if ($scope.send.currency_code === 'LAT') {
                currentKey = key;
              }
